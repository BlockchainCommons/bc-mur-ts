 ...
 *
 * Usage:
 *   bun scripts/api-report.ts --local   # (re)generate api/multipart-ur.api.md
 ...
 * (api/multipart-ur.api.md) is the reviewable record of the public surface: every
...
try {
  ...
  if (result.apiReportChanged && !local) {
    console.error(
      "Public API surface changed but api/multipart-ur.api.md was not updated.\n" +
  ...
  console.log(
    local
      ? "API report (api/multipart-ur.api.md) is up to date."
