# Migrating from `@bcts/multipart-ur` to `@blockchaincommons/multipart-ur`

`@blockchaincommons/multipart-ur` is the canonical home of this library. It was extracted from the
[`paritytech/bcts`](https://github.com/paritytech/bcts) monorepo, where it was
published as `@bcts/multipart-ur`, into its own Blockchain Commons repository at
...
migration is a rename. `@bcts/multipart-ur` remains published for one beta cycle as a
...
- [ ] Rewrite import specifiers: `@bcts/multipart-ur` becomes `@blockchaincommons/multipart-ur`.
...
```diff
- import { /* ... */ } from "@bcts/multipart-ur";
+ import { /* ... */ } from "@blockchaincommons/multipart-ur";
```

```diff
  "dependencies": {
-   "@bcts/multipart-ur": "^1.0.0-beta.6"
+   "@blockchaincommons/multipart-ur": "^1.0.0-beta.1"
  }
```

## 2. Version numbering restarts

`@bcts/multipart-ur` versions moved in lockstep with every other package in the
...
`@bcts/multipart-ur` shipped an additional IIFE bundle exposed through the `browser`
...
- The wire format. Encodings produced by `@bcts/multipart-ur` decode here, and the reverse.
