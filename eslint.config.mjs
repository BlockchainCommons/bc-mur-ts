...
/*
 * Strict, type-checked ESLint flat config for the @blockchaincommons/multipart-ur library.
 */
...
export default [
  ...
  {
    files: ["src/bin/**/*.ts", "src/cli/**/*.ts", "src/prores.ts", "src/svg.ts"],
    languageOptions: {
      globals: {
        process: "readonly",
        console: "readonly",
  ...
  {
    files: ["src/cli/**/*.ts", "src/bin/**/*.ts", "src/frames.ts"],
    rules: {
      "@typescript-eslint/strict-boolean-expressions": "off",
